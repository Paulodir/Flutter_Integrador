...
import 'racao_helper.dart';
import 'inseminaPartos_helper.dart';
...
class Api {
  ...
  Future<bool> deletarBovino(String codigoBovino, String token) async {
    http.Response response = await http.delete(
        ...
        headers: {'token': token, 'Content-Type': 'application/json'});
    if (response.statusCode == 200) {
      //print(BASE_URL + "Bovino/" + codigoBovino);
      return true;
    } else {
      return false;
    }
  ...
  Future<Ordenha> atualizarOrdenha( Ordenha ordenha, String token) async {
    http.Response response = await http.put(
        ...
        headers: {'token': token, 'Content-Type': 'application/json'});
    print('dentro Api Inseminacao '+Ordenha.fromJson(json.decode(response.body)).toString());
    if (response.statusCode == 200) {
      //print(response.body);
  ...
  Future<Inseminacao> atualizarInseminacao( Inseminacao inseminacao, String token) async {
    http.Response response = await http.put(
        ...
        headers: {'token': token, 'Content-Type': 'application/json'});
    print('dentro Api Inseminacao '+Inseminacao.fromJson(json.decode(response.body)).toString());
    if (response.statusCode == 200) {
      //print(response.body);
    ...
    http.Response response = await http.put(
        ...
        body: jsonEncode({
          ...
          "nascido": parto.nascido
             ...
             headers: {'token': token, 'Content-Type': 'application/json'});
    print('dentro Api '+Parto.fromJson(json.decode(response.body)).toString());
  ...
  Future<List<Home>> resumo(String token) async {
    http.Response response = await http.get(BASE_URL + 'Bovino/Resumo',
        headers: {'token': token, 'Content-Type': 'application/json'});
    if (response.statusCode == 200) {
      List<Home> resumo = json.decode(response.body).map<Home>((map) {
  ...
  Future<List<Racao>> alimentacao(String token) async {
    http.Response response = await http.get(BASE_URL + 'Bovino/Alimentacao',
        headers: {'token': token, 'Content-Type': 'application/json'});
    if (response.statusCode == 200) {
      List<Racao> alimentacao = json.decode(response.body).map<Racao>((map) {
        return Racao.fromJson(map);
      }).toList();
      //print('dentro da api $racas');
      return alimentacao;
    } else {
  ...
  Future<List<InseminaPartos>> relacao(String token) async {
    http.Response response = await http.get(BASE_URL + 'Inseminacao/Parto',
        headers: {'token': token, 'Content-Type': 'application/json'});
    if (response.statusCode == 200) {
      List<InseminaPartos> relacao = json.decode(response.body).map<InseminaPartos>((map) {
        return InseminaPartos.fromJson(map);
      }).toList();
      //print('dentro da api $racas');
      return relacao;
    } else {
